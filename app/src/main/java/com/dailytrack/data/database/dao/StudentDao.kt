...
interface StudentDao {
    ...
    @Query("SELECT * FROM students WHERE isActive = 1 ORDER BY rollNo ASC")
    fun getAllActiveStudents(): Flow<List<Student>>
    
    @Query("SELECT * FROM students WHERE id = :studentId")
    suspend fun getStudentById(studentId: String): Student?
    
    @Query("SELECT * FROM students WHERE rollNo = :rollNo AND isActive = 1")
    suspend fun getStudentByRollNo(rollNo: String): Student?
    
    @Query("SELECT * FROM students WHERE name LIKE '%' || :searchQuery || '%' AND isActive = 1 ORDER BY rollNo ASC")
    ...
    @Query("SELECT COUNT(*) FROM students WHERE rollNo = :rollNo AND isActive = 1")
